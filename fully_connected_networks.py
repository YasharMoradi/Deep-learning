...
class Linear(object):
    ...
    def forward(x, w, b):
        ...
        N = x.shape[0]
        x_ = np.reshape(x, (N, -1))
        out =torch.tensor(np.dot(x_, w)) +b
        ######################################################################
        #                        END OF YOUR CODE                            #
        ######################################################################
        cache = (x, w, b)
    ...
    def backward(dout, cache):
        ...
        N = x.shape[0]
        dx = np.dot(dout,w.T)
        dx = torch.tensor(dx.reshape(x.shape))
        x_ = x.reshape(N,-1)
        dw = torch.tensor(np.dot(x_.T,dout))
        db = torch.tensor(np.sum(np.array(dout),axis=0))
        ##################################################
        #                END OF YOUR CODE                #
        ##################################################
        return dx, dw, db
...
class ReLU(object):
    ...
    def forward(x):
        ...
        # Replace "pass" statement with your code
        shape = x.shape
        out = torch.maximum(x,torch.zeros(shape))
        ###################################################
        #                 END OF YOUR CODE                #
        ###################################################
        cache = x
...
class Linear_ReLU(object):
    ...
    def forward(x, w, b):
        ...
        # Replace "pass" statement with your code
        lin = Linear()
        fc_out, fc_cache = lin.forward(x, w, b)
        lin_rel = ReLU()
        out, relu_cache = lin_rel.forward(fc_out)
    ...
    def backward(dout, cache):
        ...
        # Replace "pass" statement with your code
        lin_rel = ReLU()
        dxx = lin_rel.backward(dout, cache[1])
        lin = Linear()
        dx, dw, db = lin.backward(dxx, cache[0])
        ######################################################################
        #                END OF YOUR CODE                                    #
        ######################################################################
        return dx, dw, db
...
def softmax_loss(x, y):
    ...
    S = x.shape[1]
    N = x.shape[0]
    probs = np.array(torch.div(torch.exp(x),torch.exp(x).sum(dim =1).repeat(S,1).T))
...
class TwoLayerNet(object):
    ...
    def __init__(self, input_dim=3*32*32, hidden_dim=100, num_classes=10,
        ...
        ###################################################################
        # Replace "pass" statement with your code
        self.params['W1'] = torch.tensor(weight_scale * np.random.randn(input_dim, hidden_dim))
        self.params['b1'] = torch.tensor(np.zeros(hidden_dim))
        self.params['W2'] = torch.tensor(weight_scale * np.random.randn(hidden_dim, num_classes))
        self.params['b2'] = torch.tensor(np.zeros(num_classes))
        ###############################################################
        #                            END OF YOUR CODE                 #
        ###############################################################
    ...
    def loss(self, X, y=None):
        ...
        lin = Linear()
        lin_rel = Linear_ReLU()
        h1,cach1 = lin_rel.forward(X,W1,b1)
        scores,cach2 = lin.forward(h1,W2,b2)
        ##############################################################
        #                     END OF YOUR CODE                       #
        ##############################################################
        ...
        loss = loss + reg_loss
        dout1, dw2, db2 = lin.backward(dout,cach2)
        dx1, dw1, db1 = lin_rel.backward(dout1,cach1) 
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        #######################################################################
        # Replace "pass" statement with your code
        num_dim = [input_dim] + hidden_dims + [num_classes]
        for i in range(1, len(num_dim)):
            self.params['W{}'.format(i)] = weight_scale * torch.randn(num_dim[i-1], num_dim[i], dtype=dtype, device=device)
            self.params['b{}'.format(i)] = torch.zeros(num_dim[i], dtype=dtype, device=device)
        #######################################################################
        #                         END OF YOUR CODE                            #
        #######################################################################
    ...
    def loss(self, X, y=None):
        ...
        for i in range(1, self.num_layers+1):
            ...
            if i == self.num_layers:
                out,cach = lin.forward(out,W,b)
            else:
                out,cach = lin_rel.forward(out,W,b)
            cach_list.append(cach)
        scores = out   
        #################################################################
        #                      END OF YOUR CODE                         #
        #################################################################
        ...
        for i in range(self.num_layers,0,-1):
            cach = cach_list[i-1]
            if i == self.num_layers:
                dout, dw, db = lin.backward(dout,cach)
            else:
                dout, dw, db = lin_rel.backward(dout,cach)
